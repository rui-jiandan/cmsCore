...
namespace LinCms.Middleware
{
    ...
    public class AopCacheIntercept : IInterceptor
    {
        ...
        public void Intercept(IInvocation invocation)
        {
            try
            {
                ...
                if (cacheAttrObj is CacheableAttribute cacheAttr)
                {
                    ...
                    string cacheValue = _cacheHelp.Get(cacheKey);
                    if (cacheValue != null)
                    {
                    ...
                    else
                    {
                        ...
                        _cacheHelp.Set(cacheKey, JsonConvert.SerializeObject(invocation.ReturnValue),TimeSpan.FromSeconds( ExpireSeconds));
                    }
                    return;
                }
        ...
        private async Task<T> InterceptAsync<T>(string cacheKey, Task<T> task)
        {
            ...
            int ExpireSeconds = int.Parse(_configuration["Cache:ExpireSeconds"].ToString());
            await _cacheHelp.SetAsync(cacheKey, JsonConvert.SerializeObject(result), TimeSpan.FromSeconds(ExpireSeconds));
            return result;
        }
