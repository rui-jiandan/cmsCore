...
namespace LinCms.Startup
{
    public static class DependencyInjectionExtensions
    {
        ...
        public static void AddCacheCore(this IServiceCollection services, IConfiguration configuration)
        {
            IConfigurationSection cacheType = configuration.GetSection("Cache:CacheType");
            if (Enum.TryParse(cacheType.Value, out CacheType c))
            {
                if (!Enum.IsDefined(typeof(CacheType), c))
                {
                    Log.Error($"CAP配置CAP:DefaultStorage:{cacheType.Value}无效");
                }                
                switch (c)
                {
            ...
            else
            {
                services.AddMemoryCache();
                services.AddSingleton<ICache, MemoryCache>();
                Log.Error($@"Cache:CacheType:{cacheType.Value}无效，只支持Memory=0,Redis=1;已自动启用Memory");
            }
        }
        #endregion
